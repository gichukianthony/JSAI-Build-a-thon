  ...
  override render() {
    return html`
      ...
      <section id="chat__containerWrapper" class="chat__containerWrapper">
        ...
        : html`
                <teaser-list-component
                  .prompts="${teaserListTexts.DEFAULT_PROMPTS}"
                  .heading="${this.isCustomBranding ? '' : teaserListTexts.HEADING_ASK}"
        ...
        ? html`<loading-indicator .text="${globalConfig.LOADING_INDICATOR_TEXT}"></loading-indicator>`
        : ''}
            ${this.chatController.processingMessage?.error
        ? html`
                  <div class="error-message">
                    <p>${this.chatController.processingMessage.error.message}</p>
                  </div>
                `
        : ''}
          </div>
        ...
        >
          <form
            id="chat-form"
            @submit="${(event: Event) => this.handleUserChatSubmit(event)}"
          >
            <div class="chatbox-wrapper">
              <input
                ...
                ?disabled="${this.isDisabled}"
                @input="${(e: InputEvent) => (this.currentQuestion = (e.target as HTMLInputElement).value)}"
                @keydown="${this.handleKeydown}"
              />
              <voice-input-button
                .isDisabled="${this.isDisabled}"
                @on-vocal-input="${this.handleVoiceInput}"
              ></voice-input-button>
              <button
                class="chatbox__button"
                id="chat-button"
                type="submit"
                ?disabled="${this.isDisabled || !this.currentQuestion.trim()}"
              >
                ${unsafeSVG(iconSend)}
              </button>
            </div>
